...
# Add project root to path for `from src... import ...` style imports
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, PROJECT_ROOT)

# Workspace root (fin/) holds configs/, data/, output/ after the reorganization
WORKSPACE_ROOT = os.path.dirname(os.path.dirname(PROJECT_ROOT))

# Change to workspace root so relative config/data/output paths resolve
...
import findata.simulated as sim_params
...
def create_lifecycle_visualization(acc_paths, dec_paths, acc_values, dec_values,
    ...
    filepath = 'output/plots/test/mc_validation_lifecycle.png'
...
config = SystemConfig.from_json(os.path.join(WORKSPACE_ROOT, 'configs/portfolio/test_simple_buyhold.json'))
...
mean_returns_df = pd.read_csv(config.simulated_mean_returns_file, index_col=0)

# Load covariance matrices from file as 2D array (2 regimes × n_assets × n_assets flattened)
cov_matrices_2d = np.loadtxt(config.simulated_cov_matrices_file)
...
plot_covariance_evolution(
    ...
    output_path='output/plots/test/mc_validation_covariance.png'
)
...
print("\nVisualizations:")
print(f"  ✓ Lifecycle: output/plots/test/mc_validation_lifecycle.png")
print(f"  ✓ Covariance: output/plots/test/mc_validation_covariance.png")
print("=" * 80)
