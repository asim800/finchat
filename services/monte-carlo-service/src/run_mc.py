...
# Workspace root (fin/) - where configs/, data/, output/ live
WORKSPACE_ROOT = os.path.dirname(os.path.dirname(PROJECT_ROOT))

from .config import SystemConfig
from .montecarlo import MCPathGenerator
from .montecarlo.lifecycle import run_accumulation_mc, run_decumulation_mc
...
def _get_tickers_from_config(config: SystemConfig) -> list:
    ...
    if not os.path.isabs(ticker_file):
        ticker_file = os.path.join(WORKSPACE_ROOT, ticker_file)
...
def run_mc_simulation(
    ...
    if not os.path.isabs(mean_file):
        mean_file = os.path.join(WORKSPACE_ROOT, mean_file)
    if not os.path.isabs(cov_file):
        cov_file = os.path.join(WORKSPACE_ROOT, cov_file)
